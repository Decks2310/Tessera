...
void StringResonator::prepare(const juce::dsp::ProcessSpec& spec) {
    ...
    for (size_t ch = 0; ch < spec.numChannels; ++ch) {
        ...
        // Initialize Dispersion Filter (AllpassTPTFilter)
        channelDispersionFilters[ch].prepare(monoSpec);
        // FIX: Initialize using JUCE 8 API. This resolves the 'allpass' errors (C2838/C2065).
        juce::dsp::AllpassTPTFilterParameters dispersionParams;
        dispersionParams.cutoffFrequency = 1000.0f; // Safe default
        dispersionParams.Q = 0.707f; // Safe default
        channelDispersionFilters[ch].setParameters(dispersionParams);
    }
...
void StringResonator::process(const juce::dsp::AudioBlock<float>& excitationBlock, juce::dsp::AudioBlock<float>& outputBlock,
    ...
    // Update parameters (Efficient TPT updates)
    for (int ch = 0; ch < numChannels; ++ch) {
        if ((size_t)ch >= channelDelays.size()) continue;
        ...
        // FIX: Update Dispersion Filter (AllpassTPTFilter) using JUCE 8 API.
        // This resolves the second 'setQ' error (C2039).
        auto dispersionParams = channelDispersionFilters[ch].getParameters();
        dispersionParams.cutoffFrequency = tuneHz;
        dispersionParams.Q = dispersionQ;
        channelDispersionFilters[ch].setParameters(dispersionParams);
    }
    ...
    for (int i = 0; i < numSamples; ++i) {
        ...
        for (int ch = 0; ch < numChannels; ++ch) {
            ...
            // 2. Apply filters in the feedback loop (MONO processing, channel 0)
            // Damping (Brightness)
            float dampedSample = channelDampingFilters[ch].processSample(0, delayedSample);

            // Dispersion (Inharmonicity)
            float dispersedSample = channelDispersionFilters[ch].processSample(0, dampedSample);
