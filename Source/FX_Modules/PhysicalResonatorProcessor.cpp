...
void ExcitationGenerator::prepare(const juce::dsp::ProcessSpec& spec) {
    ...
    // FIX: Initialize colorFilter using JUCE 8 API (Direct setters).
    // The Parameters struct approach is removed in JUCE 8.
    colorFilter.setType(juce::dsp::StateVariableTPTFilterType::lowpass);
    colorFilter.setCutoffFrequency(5000.0f);

    // JUCE 8 StateVariableTPTFilter::setResonance uses damping factor R = 1/(2Q).
    // For Butterworth (Q=0.707), R = 1/(2*0.707) = 0.707.
    colorFilter.setResonance(0.707f);
...
void ExcitationGenerator::process(const juce::dsp::AudioBlock<float>& inputBlock, juce::dsp::AudioBlock<float>& outputExcitationBlock, const ExcitationParams& params) {
    ...
    // FIX: Update colorFilter using JUCE 8 API (Direct setters).
    // getParameters() and setParameters() are removed.
    colorFilter.setCutoffFrequency(5000.0f);
    colorFilter.setResonance(0.707f);
...
void ModalResonator::process(const juce::dsp::AudioBlock<float>& excitationBlock,
    ...
    // FIX: Update using JUCE 8 API (Direct setters).
    for (int ch = 0; ch < numChannels; ++ch) {
        if ((size_t)ch >= channelFilters.size()) continue;
        auto& bank = channelFilters[(size_t)ch];
        for (int m = 0; m < NUM_MODES; ++m) {
            // getParameters() and setParameters() are removed.
            bank[m].setCutoffFrequency(modeFreqs[m]);
            // JUCE 8 StateVariableTPTFilter::setResonance expects R = 1/(2Q).
            bank[m].setResonance(1.0f / (2.0f * modeQs[m]));
        }
    }
...
void SympatheticStringResonator::prepare(const juce::dsp::ProcessSpec& spec) {
    ...
    for (size_t ch = 0; ch < spec.numChannels; ++ch) {
        for (int s = 0; s < NUM_STRINGS; ++s) {
            ...
            // FIX: Initialize FirstOrderTPTFilter using JUCE 8 API (Direct setters).
            channelFilters[ch][s].setType(juce::dsp::FirstOrderTPTFilterType::lowpass);
            channelFilters[ch][s].setCutoffFrequency(20000.0f); // Default high cutoff
...
void SympatheticStringResonator::process(const juce::dsp::AudioBlock<float>& excitationBlock, juce::dsp::AudioBlock<float>& outputBlock,
    ...
    for (int ch = 0; ch < numChannels; ++ch) {
        if ((size_t)ch >= channelFilters.size()) continue;
        for (int s = 0; s < NUM_STRINGS; ++s) {
            // FIX: Update FirstOrderTPTFilter using JUCE 8 API (Direct setter).
...
void StringResonator::prepare(const juce::dsp::ProcessSpec& spec) {
    ...
    for (size_t ch = 0; ch < spec.numChannels; ++ch) {
        ...
        // FIX: Initialize using JUCE 8 API (Direct setters).
        channelDampingFilters[ch].setType(juce::dsp::FirstOrderTPTFilterType::lowpass);
        channelDampingFilters[ch].setCutoffFrequency(20000.0f);
        ...
        // In JUCE 8, use the public 'coefficients' member instead of getCoefficients().
        channelDispersionFilters[ch].coefficients = juce::dsp::IIR::Coefficients<float>::makeAllPass(sampleRate, 1000.0f, 0.707f);
    }
