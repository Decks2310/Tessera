...
// ... (All existing editor implementations remain unchanged) ...
//==============================================================================
...
//==============================================================================
// HelicalDelaySlotEditor Implementation
//==============================================================================
HelicalDelaySlotEditor::HelicalDelaySlotEditor(juce::AudioProcessorValueTreeState& apvts, const juce::String& paramPrefix)
    : SlotEditorBase(apvts, paramPrefix),
    timeKnob(apvts, paramPrefix + "HELICAL_TIME", "Time"),
    pitchKnob(apvts, paramPrefix + "HELICAL_PITCH", "Pitch"),
    feedbackKnob(apvts, paramPrefix + "HELICAL_FEEDBACK", "Feedback"),
    degradeKnob(apvts, paramPrefix + "HELICAL_DEGRADE", "Degrade"),
    textureKnob(apvts, paramPrefix + "HELICAL_TEXTURE", "Texture"),
    mixKnob(apvts, paramPrefix + "HELICAL_MIX", "Mix")
{
    addAndMakeVisible(timeKnob);
    addAndMakeVisible(pitchKnob);
    addAndMakeVisible(feedbackKnob);
...
void HelicalDelaySlotEditor::resized()
{
    auto bounds = getLocalBounds().reduced(10);
    juce::FlexBox fb;
    fb.flexWrap = juce::FlexBox::Wrap::wrap;
    ...
    fb.items.add(LayoutHelpers::createFlexKnob(feedbackKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(degradeKnob, basis));
...
//==============================================================================
// ChronoVerbSlotEditor Implementation
//==============================================================================
ChronoVerbSlotEditor::ChronoVerbSlotEditor(juce::AudioProcessorValueTreeState& apvts, const juce::String& paramPrefix)
    : SlotEditorBase(apvts, paramPrefix),
    sizeKnob(apvts, paramPrefix + "CHRONO_SIZE", "Size"),
    decayKnob(apvts, paramPrefix + "CHRONO_DECAY", "Decay"),
    diffusionKnob(apvts, paramPrefix + "CHRONO_DIFFUSION", "Diffusion"),
    dampingKnob(apvts, paramPrefix + "CHRONO_DAMPING", "Damping"),
    modulationKnob(apvts, paramPrefix + "CHRONO_MODULATION", "Modulation"),
    balanceKnob(apvts, paramPrefix + "CHRONO_BALANCE", "Balance"),
    mixKnob(apvts, paramPrefix + "CHRONO_MIX", "Mix")
{
...
void ChronoVerbSlotEditor::resized()
{
    auto bounds = getLocalBounds().reduced(10);
    juce::FlexBox fb;
    fb.flexWrap = juce::FlexBox::Wrap::wrap;
    ...
    // Layout with 4 knobs on top, 3 on bottom for a balanced look
    float basis = (float)bounds.getWidth() / 4.0f;
    if (basis < LayoutHelpers::minKnobWidth)
        basis = (float)bounds.getWidth() / 3.0f;

    fb.items.add(LayoutHelpers::createFlexKnob(sizeKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(decayKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(diffusionKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(dampingKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(modulationKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(balanceKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(mixKnob, basis));
...
//==============================================================================
// TectonicDelaySlotEditor Implementation
//==============================================================================
TectonicDelaySlotEditor::TectonicDelaySlotEditor(juce::AudioProcessorValueTreeState& apvts, const juce::String& paramPrefix)
    : SlotEditorBase(apvts, paramPrefix),
    lowTimeKnob(apvts, paramPrefix + "TECTONIC_LOW_TIME", "Low Time"),
    midTimeKnob(apvts, paramPrefix + "TECTONIC_MID_TIME", "Mid Time"),
    highTimeKnob(apvts, paramPrefix + "TECTONIC_HIGH_TIME", "High Time"),
    feedbackKnob(apvts, paramPrefix + "TECTONIC_FEEDBACK", "Feedback"),
    lowMidCrossKnob(apvts, paramPrefix + "TECTONIC_LOMID_CROSS", "L/M Cross"), // FIXED ID
    midHighCrossKnob(apvts, paramPrefix + "TECTONIC_MIDHIGH_CROSS", "M/H Cross"), // FIXED ID
    decayDriveKnob(apvts, paramPrefix + "TECTONIC_DECAY_DRIVE", "Drive"),
    decayTextureKnob(apvts, paramPrefix + "TECTONIC_DECAY_TEXTURE", "Texture"),
    decayDensityKnob(apvts, paramPrefix + "TECTONIC_DECAY_DENSITY", "Density"),
    decayPitchKnob(apvts, paramPrefix + "TECTONIC_DECAY_PITCH", "Pitch"),
    mixKnob(apvts, paramPrefix + "TECTONIC_MIX", "Mix"),
    linkButton("Link")
{
    // Add delay time controls
    addAndMakeVisible(lowTimeKnob);
    addAndMakeVisible(midTimeKnob);
    addAndMakeVisible(highTimeKnob);
    
    // Add crossover controls
    addAndMakeVisible(lowMidCrossKnob);
    addAndMakeVisible(midHighCrossKnob);
    
    // Add feedback and mix
    addAndMakeVisible(feedbackKnob);
    addAndMakeVisible(mixKnob);
    
    // Add decay synthesis controls
    addAndMakeVisible(decayDriveKnob);
    addAndMakeVisible(decayTextureKnob);
    addAndMakeVisible(decayDensityKnob);
    addAndMakeVisible(decayPitchKnob);
    
    // Add link button
    addAndMakeVisible(linkButton);
    linkButton.setClickingTogglesState(true);
    linkAttachment = std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment>(
        apvts, paramPrefix + "TECTONIC_LINK", linkButton);
}

void TectonicDelaySlotEditor::resized()
{
    auto bounds = getLocalBounds().reduced(10);
    
    // Reserve space for link button at the top
    auto linkArea = bounds.removeFromTop(30);
    linkButton.setBounds(linkArea.withWidth(80).withCentre(juce::Point<int>(linkArea.getCentreX(), linkArea.getCentreY())));
    ...
    // Calculate basis for 3 columns layout
    float basis = (float)bounds.getWidth() / 3.0f;
    if (basis < LayoutHelpers::minKnobWidth)
        basis = (float)bounds.getWidth() / 2.0f;

    // Row 1: Delay times
    fb.items.add(LayoutHelpers::createFlexKnob(lowTimeKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(midTimeKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(highTimeKnob, basis));
    
    // Row 2: Crossovers and feedback
    fb.items.add(LayoutHelpers::createFlexKnob(lowMidCrossKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(midHighCrossKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(feedbackKnob, basis));
    
    // Row 3: Decay synthesis controls
    fb.items.add(LayoutHelpers::createFlexKnob(decayDriveKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(decayTextureKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(decayDensityKnob, basis));
    
    // Row 4: Pitch and mix
    fb.items.add(LayoutHelpers::createFlexKnob(decayPitchKnob, basis));
    fb.items.add(LayoutHelpers::createFlexKnob(mixKnob, basis));
